...
const Checkout = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    const orderData = {
      _type: 'order', // Define your order type in Sanity
      ...
      cartItems: cartItems.map(item => ({
        _type: 'reference',
        _ref: item.product._id, // Reference to the product in your Sanity dataset
      })),
    ...
    try {
      ...
      setLoading(false);
      setOrderSuccess(true);  // Show success message if order is placed successfully
    } catch (error) {
      setLoading(false);
    ...
    <div className="max-w-screen-lg mx-auto p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-6">
        ...
        <form className="space-y-6">
          ...
          {/* Province */}
          <div>
            <label htmlFor="province" className="block text-sm font-medium text-gray-700">Province (optional)</label>
            <input
              id="province"
              name="province"
              type="text"
        ...
        <div className="p-6">
          ...
          {cartItems.length === 0 ? (
            ...
            cartItems.map((item) => (
              <div key={item.product._id} className="grid grid-cols-4 gap-4 mt-4 items-center text-gray-800">
                <p className="text-[#333333] text-sm sm:text-base lg:text-lg xl:text-xl col-span-2 sm:col-span-2 font-medium">
                  {item.product.title} x {item.count}
                </p>
                <p className="text-[#333333] text-sm sm:text-base lg:text-lg xl:text-xl text-center font-medium">
          ...
          <div className="text-[#9F9F9F] py-4">
            <div className="group mb-3">
              <label
                className={`inline-flex cursor-pointer items-center gap-3 mb-3 duration-300 ${selectedPayment === "bank" ? "text-black" : ""}`}
                htmlFor="bank"
              >
                <input
                  className="peer sr-only"
            ...
            <div className="group">
              <label
                className={`inline-flex cursor-pointer items-center gap-3 mb-3 duration-300 ${selectedPayment === "cash" ? "text-black" : ""}`}
                htmlFor="cash"
              >
                <input
                  className="peer sr-only"
          ...
          <div className="flex justify-end">
            <button
              onClick={handlePlaceOrder}
              className="bg-black text-white py-3 px-12 rounded-md font-medium"
              disabled={loading}
            >
              {loading ? <PulseLoader size={10} color={"#fff"} /> : "Place Order"}
            </button>
          </div>
